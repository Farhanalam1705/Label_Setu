  ...
  return (
    <div className="relative bg-slate-100 rounded-xl overflow-hidden border border-slate-300 p-2">
      {src ? (
        ...
        <div className="flex items-center justify-center h-64 text-slate-500">No image available</div>
      )}
      {regions.map((region) => (
        <div
          key={region.id}
        ...
        >
          <span className="absolute -top-4 left-0 text-xs font-mono bg-slate-800 text-cyan-200 px-1 rounded">{region.label}</span>
        </div>
      ))}
    </div>
  );
