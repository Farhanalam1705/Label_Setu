...
import { MoreVertical, Eye, FileText, Shield, ArrowRight, Trash2 } from 'lucide-react';
...
export const InspectionRow = ({
  ...
  const handleDelete = () => {
    ...
    if (onDeleteInspection) {
      onDeleteInspection(inspection.inspectionId);
    }
  };

  return (
    <tr className={`hover:bg-slate-50/80 transition-colors border-b border-slate-100 group text-xs ${isSelected ? 'bg-cyan-50/30' : ''}`}>
      {/* Selection Checkbox */}
      <td className="py-3.5 pl-4 pr-1 text-center w-8">
        <input
          type="checkbox"
          checked={!!isSelected}
          onChange={() => onToggleSelect && onToggleSelect(inspection.inspectionId)}
            ...
            {isMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-lg border border-slate-200 py-1.5 z-30 animate-in fade-in zoom-in-95">
                ...
                >
                  ...
                  <span>{t('deleteRecord', 'Delete Record')}</span>
                </button>
              </div>
            )}
          </div>
