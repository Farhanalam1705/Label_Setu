...
export const ProfileSettings = ({ profile, onSaveProfile }) => {
  ...
  const handleSave = (e) => {
    ...
    addToast({
      ...
      message: t('profileSavedDesc', 'Profile updated successfully.'),
      type: 'success',
    });
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-300 uppercase">
                  {t('activeOfficer', 'Active Officer')}
                </span>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h4 className="text-xs font-bold text-slate-900">{t('accountSession', 'Account Session')}</h4>
          <p className="text-[11px] text-slate-500">{t('accountSessionDesc', 'Sign out of this workstation session.')}</p>
        </div>
