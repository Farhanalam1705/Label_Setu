...
export const CustomerComplaints = () => {
  ...
  const handleViewComplaintReport = () => {
    const cId = selectedComplaintDetail?.complaintId || selectedComplaintDetail?.id;
    if (cId) navigate(`/customer/complaints/${cId}/report`);
  };
      ...
      {selectedComplaintDetail && (
        ...
        >
          <div onClick={(event) => event.stopPropagation()} className="bg-[#0f1b2d] border border-slate-700 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col text-white">
            ...
            <div className="mx-6 mb-4 rounded-xl border border-slate-700 bg-[#142237] p-3 text-xs">
              {selectedComplaintDetail.reportAvailable ? (
                <div className="flex items-center justify-between gap-3"><div><p className="font-bold text-emerald-300">Complaint Report Available</p><p className="mt-0.5 text-slate-400">Officer-generated report is ready to view and download.</p></div><FileText className="h-5 w-5 shrink-0 text-emerald-400" /></div>
              ) : (
                <div><p className="font-bold text-slate-200">Complaint Report</p><p className="mt-0.5 text-slate-400">Report has not been generated by the reviewing officer yet.</p></div>
              )}
            </div>
            ...
            <div className="p-4 px-6 border-t border-slate-800 bg-[#0a1526] flex items-center justify-between sticky bottom-0">
              ...
              {selectedComplaintDetail.reportAvailable && <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => handleViewComplaintReport()}
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/customer/complaints/${selectedComplaintDetail.complaintId || selectedComplaintDetail.id}/report?download=1`)}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition-colors cursor-pointer"
                >
