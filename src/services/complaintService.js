...
export const generateComplaintReport = (complaintId, officer = {}) => {
  const complaint = getComplaintById(complaintId);
  if (!complaint) return null;
  if (complaint.reportAvailable) return complaint;
  ...
  return updateComplaint(complaintId, {
    ...
    generatedBy: {
      ...
      name: officer.name || 'Authorized Officer',
      designation: officer.designation || 'Enforcement Official',
    },
    generatedInspectionId: complaint.inspectionId || '',
    timeline: [
      ...(complaint.timeline || []),
      { status: 'REPORT_GENERATED', title: 'Complaint Report Generated', date: now, note: `Report ${reportId} generated by ${officer.name || 'Authorized Officer'}.`, by: officer.name || 'Officer' },
    ],
  });
};
